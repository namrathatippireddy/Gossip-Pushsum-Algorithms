  ...
  def main do
    ...
    node_list =
      Enum.map(1..num_nodes, fn n ->
        ...
        # commented the above unblock myself with spliting and all for the sake of pushsum initialization of s
        n_string = Integer.to_string(n)
        n = String.to_atom(n_string)
      end)
  ...
  def spawn_actors(node_list) do
    ...
    Enum.map(node_list, fn n ->
      if n == random_initial_node do
        {:ok, actor} = GenServer.start_link(GossipActor, ["rumor", n], name: n)
        actor
      else
  ...
  def spawn_pushsum_actors(node_list) do
    ...
    Enum.map(node_list, fn n ->
      if n == random_initial_node do
        ...
        s_string = Atom.to_string(n)
        s_integer = String.to_integer(s_string)
